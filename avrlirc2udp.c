...
data_loop(int from, int tcp, char *host, int port)
{
    ...
    while (1) {
        ...
        /* in my experience, this results from a USB serial device being unplugged */
        if (n == 0)
            return;
        ...
        /*  The MSB is flag which defines a SPACE if set for either 2 byte or 4 byte format  */
        is_space = (b[1] & 0x80);
        len = ((b[1] << 8) | b[0]) & 0x7fff;
        ...
        if (debug) {
            fprintf(stderr, "%s 0x%08x (%d)\n",
                    is_space ? "space": "pulse", len_raw, len);
        }
        ...
        if (debug != DEBUG_ONLY)  { // sending to host
            ...
            if (to >= 0) {
                if (n == 2) {
                    ...
                    fprintf(stderr, "ERR: data_loop: bad n=%d\n", n);
                }
            }
        }
